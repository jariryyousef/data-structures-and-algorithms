...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  starWarsArr.sort((z, x) => {
    return x.height - z.height;
  });
  return starWarsArr;
};
...
const listFoods = (recipe) => {
  let resultarr = [];
  // Solution code here...
  let foodsarr = recipe.ingredients;
  for (let i=0; i< foodsarr.length ;i++){
    foodsarr[i]= foodsarr[i].substr(foodsarr[i].indexOf(' ') + 1);
    resultarr.push(foodsarr[i]);
  }

  for (let i=0; i< resultarr.length ;i++){
    resultarr[i]= resultarr[i].substr(resultarr[i].indexOf(' ') + 1);
  }
  return resultarr;
};
/* ------------------------------------------------------------------------------------------------
